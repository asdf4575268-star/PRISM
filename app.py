...
def restore_from_google():
    ...
    for col in df.columns:
        ...
        elif "creator" in lower or "만든" in lower or "작가" in lower or "감독" in lower: col_map["creator"] = col
        # [수정] 공개일, 개봉일, 출판일 등 다양한 표현을 모두 잡도록 추가
    ...
    with sqlite3.connect(DB_NAME) as conn:
        ...
        for _, row in df.iterrows():
            # rel_date가 정상적으로 매핑되었는지 확인 후 삽입
            rel_val = str(row.get(col_map.get("rel_date"), "")).strip()
            # 'nan' 문자열 방지
            rel_val = "" if rel_val.lower() == "nan" else rel_val
            ...
            """, (
                ...
                rel_val,  # [수정] 이 자리에 공개일 데이터가 정확히 들어가는지 확인
