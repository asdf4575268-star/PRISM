...
st.markdown("""
    ...
    .num-text { font-size: 60px !important; font-family: 'Jolly Lodger', cursive; margin: 0; line-height: 1; display: flex; align-items: baseline; }
...
with t_col2:
    ...
    try:
        with open(DB_NAME, "rb") as f:
            st.download_button(label="📥 DB 백업", data=f.read(), file_name=f"prism_backup_{date.today()}.db", use_container_width=True)
    except: pass
...
with tab_write:
    ...
    with cr:
        ...
        if st.button("✅ 저장", use_container_width=True):
            if title:
                with sqlite3.connect(DB_NAME) as conn:
                    conn.execute("INSERT INTO archive (category, title, creator, rel_date, summary, brief, note, img_url, save_date, view_date) VALUES (?,?,?,?,?,?,?,?,?,?)",
                                 (category, title, creator, rel_date, summary, brief, note, img_url_val, str(date.today()), str(view_date)))
    ...
    if not all_df.empty:
        ...
        with sub_tabs[0]: # YEARLY
            ...
            for month in range(12, 0, -1):
                ...
                if not m_data.empty:
                    # 60px 숫자 + 30px '월'
                    st.markdown(f'<div class="num-text">{month}<span style="font-size:30px; font-family: sans-serif; margin-left: 5px;">월</span></div>', unsafe_allow_html=True)
                    cols = st.columns(6)
