...
with tab2:
    ...
    for idx, c_name in enumerate(cats):
        with sub_tabs[idx+1]:
            ...
            if not df.empty:
                # 💡 데이터를 6개씩 끊어서 순서대로 가로 배치 (정렬 꼬임 방지)
                    ...
                    for j in range(6):
                        if i + j < len(df):
                            ...
                            with cols[j]:
                                ...
                                st.markdown(f'<p class="date-text" style="font-size:12px; text-align:center; margin-bottom: 3px;">🍿 {v_date_display}</p>', unsafe_allow_html=True)
