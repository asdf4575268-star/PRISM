...
def restore_from_google():
    try:
        ...
        for col in df.columns:
            ...
            elif "venue" in lower or "장소" in lower: col_map["venue"] = col # 장소 맵핑 추가
        ...
        with sqlite3.connect(DB_NAME) as conn:
            ...
            for _, row in df.iterrows():
                ...
                """, (
                    ...
                    str(row.get(col_map.get("venue"), "")), # 장소 데이터 포함
...
def get_kopis_detail(mt20id):
    ...
    try:
        ...
        if d is not None:
            ...
            return f"{crew} / {cast}".strip(" / ") if crew or cast else "정보 없음"
    ...
    with col_txt:
        if edit_mode:
            with st.form(key=f"edit_v2_{item['id']}"):
                ...
                n_venue = st.text_input("📍 장소", value=str(item.get('venue', ''))) # 수정 모드 장소 추가
            ...
            st.markdown(f'<div style="font-size:25px; font-weight:bold; line-height:1.1;">{get_val("title")}</div>', unsafe_allow_html=True)
            st.write(f"**[{get_val('category')}]** {get_val('creator')}")
            st.write(f"**공개일:** {get_val('rel_date')}{venue_display}")
            st.markdown(f'<p style="color:gray;">🍿 감상일: {get_val("view_date") or get_val("save_date")}</p>', unsafe_allow_html=True)
            st.divider()

            for label, key in [("📝 요약", "brief"), ("📖 정보/줄거리", "summary"), ("✨ 인상 깊은 부분", "highlights")]:
                val = get_val(key)
                if val: st.markdown(f'<div style="padding:10px; border-radius:8px; border:1px solid #ddd; margin-bottom:10px;"><small style="color:#666;">{label}</small><br>{val.replace("\n", "<br>")}</div>', unsafe_allow_html=True)

            note_v = get_val('note')
            if note_v: st.markdown(f'<div style="background-color:#fff4cc; padding:15px; border-radius:10px; color:#000; border-left:5px solid #ffcc00; margin-top:10px;"><strong>💬 감상</strong><br><br>{note_v.replace("\n", "<br>")}</div>', unsafe_allow_html=True)
...
with tab1:
    ...
    with cr:
        venue = st.text_input("📍 장소 (STAGE 전용)", value=data.get('venue', ''))
        ...
        if st.button("✅ 저장", use_container_width=True):
            ...
            except: ry, rm, rd = "2026", "02", "20"; vy, vm, vd = "2026", "02", "20"
            ...
            try:
                ...
                with sqlite3.connect(DB_NAME) as conn:
                    # [수정] INSERT 문 필드 12개, 물음표 12개 완벽 일치
...
# --- TAB 2: ARCHIVE ---
with tab2:
    if st.button("🔄"): restore_from_google(); st.rerun()
    ...
    with sub_tabs[0]:
        if not all_df.empty:
            all_df['v_dt'] = pd.to_datetime(all_df['view_date'].fillna(all_df['save_date']), errors='coerce').fillna(pd.Timestamp.now())
            yearly_df = all_df.sort_values(by='v_dt', ascending=False)
            raw_years = sorted(list(yearly_df['v_dt'].dt.year.unique()), reverse=True)
            sel_y = st.selectbox("연도 선택", raw_years, key="yr_sel")
            year_data = yearly_df[yearly_df['v_dt'].dt.year == sel_y]
                    ...
                    for i in range(0, len(items), 6):
                        ...
                        for j in range(6):
                            if i + j < len(items):
                                row = items[i + j]
                                with cols[j]:
                                    try: d_display = f"{pd.to_datetime(row['view_date']).day}일"
                                    except: d_display = ""
                                    st.markdown(f'<div class="cal-img-box"><div class="badge badge-left">{row["category"]}</div><div class="badge badge-right">{d_display}</div><img src="{row["img_url"]}"></div>', unsafe_allow_html=True)
                                    if st.button(f"{row['title'][:7]}..", key=f"yr_{row['id']}", use_container_width=True): show_details(row)
    ...
    for idx, c_name in enumerate(cat_list):
        with sub_tabs[idx + 1]:
            ...
            if not cat_df.empty:
                ...
                items = cat_df.sort_values(by='sort_dt', ascending=False, na_position='last').to_dict('records')
                for i in range(0, len(items), 6):
                    cols = st.columns(6)
                    for j in range(6):
                        if i + j < len(items):
                            row = items[i + j]
                            with cols[j]:
                                try: b_date = pd.to_datetime(row['view_date']).strftime('%y.%m.%d')
                                except: b_date = ""
                                st.markdown(f'<div class="cal-img-box"><div class="badge badge-left">{b_date}</div><img src="{row["img_url"]}"></div>', unsafe_allow_html=True)
                                if st.button(f"{str(row['title'])[:7]}..", key=f"cat_{c_name}_{row['id']}", use_container_width=True): show_details(row)
