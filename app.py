...
with st.sidebar:
    ...
    if not is_admin:
        ...
        if input_pw == st.secrets["ADMIN_PASSWORD"]:
            st.session_state.user_password = input_pw; st.session_state.is_logged_in = True; st.rerun()
    else:
    ...
    if is_admin and edit_mode:
        ...
        with col_txt:
            with st.form(key=f"edit_form_{item['id']}"):
                ...
                n_venue = st.text_input("📍 장소/플랫폼/출처", value=str(item.get('venue', '')))
                n_view_date = st.date_input("🍿 감상일", value=pd.to_datetime(item.get('view_date')).date())
                n_sum = st.text_area("📖 소개/내용", value=str(item.get('summary', '')), height=150)
                n_brief = st.text_input("📝 요약", value=str(item.get('brief', '')))
                n_high = st.text_area("✨ 하이라이트", value=str(item.get('highlights', '')), height=100)
                n_note = st.text_area("💬 감상/태그", value=str(item.get('note', '')), height=100)
                if st.form_submit_button("💾 저장"):
                    get_connection().execute("""UPDATE archive SET title=?, creator=?, rel_date=?, venue=?, summary=?, brief=?, highlights=?, note=?, view_date=?, img_url=?, img_url2=? WHERE id=?""", (n_title, n_creator, n_rel, n_venue, n_sum, n_brief, n_high, n_note, str(n_view_date), n_img, n_img2, item['id'])).connection.commit()
        ...
        with col_txt:
            ...
            for label, key, color in sections:
                ...
                if content:
                    st.markdown(f'<div style="background-color: {color}; color: white; display: inline-block; padding: 2px 12px; border-radius: 12px; font-size: 0.8em; margin-bottom: 10px;">{label}</div>', unsafe_allow_html=True)
            ...
            if item.get('category') != "SCRAP":
                target_tag = f"#{item['title']}"
                all_data = get_all_data()
                related_scraps = all_data[(all_data['category'] == "SCRAP") & (all_data['note'].str.contains(target_tag, na=False))]
                if not related_scraps.empty:
                    st.markdown("#### 🔗 관련 SCRAP")
                    for _, s in related_scraps.iterrows():
                        if st.button(f"📄 {s['title']} ({s['venue']})", key=f"rel_{s['id']}", use_container_width=True):
                            st.session_state.pop_item = s; st.rerun()

# --- [6. 메인 화면] ---
logo_base64 = get_base64("logo.png")
st.markdown(f'<div style="display: flex; align-items: center; gap: 10px;"><img src="data:image/png;base64,{logo_base64}" width="90"><h1>PRISM ARCHIVE</h1></div>', unsafe_allow_html=True)
...
if is_admin and tab_w:
    with tab_w:
        ...
        if category == "SCRAP":
            ...
            if st.button("🚀 정보 추출"):
                scraped = scrape_url(url_input)
                if scraped:
                    st.session_state.api_data = {'title': scraped['title'], 'img': scraped['img'], 'venue': scraped['venue'], 'summary': scraped['summary'], 'date': str(date.today())}
                    st.rerun()
        else:
            ...
            if search_query:
                if category == "BOOKS":
                    ...
                    if res:
                        ...
                        if st.button("✨ 가져오기"):
                            b = opts[sel]; st.session_state.api_data = {'title': b['title'], 'creator': ", ".join(b['authors']), 'date': b['datetime'][:10], 'img': b.get('thumbnail', '').replace("R120x174", "R400x0"), 'venue': b.get('publisher', ''), 'summary': b.get('contents', '')}; st.rerun()
                elif category == "MUSIC":
                    ...
                    if res:
                        ...
                        sel = st.selectbox("결과", list(opts.keys()))
                        if st.button("✨ 가져오기"):
                            m = opts[sel]; st.session_state.api_data = {'title': m['title'], 'creator': m['creator'], 'date': m['date'], 'img': m['img'], 'summary': m.get('url', '')}; st.rerun()
                elif category == "STAGE":
                    ...
                    if res:
                        ...
                        sel = st.selectbox("결과", list(opts.keys()))
                        if st.button("✨ 가져오기"):
                            s = opts[sel]; combined = get_kopis_detail(s['id']); st.session_state.api_data = {'title': s['title'], 'creator': combined, 'date': s['date'], 'venue': s['venue'], 'img': s['img'], 'summary': f"https://www.kopis.or.kr/por/db/pblprfr/pblprfrView.do?menuId=MNU_00020&mt20Id={s['id']}"}; st.rerun()
                else:
                    res = search_tmdb(search_query, category)
                    if res:
                        t_k = 'title' if category == 'MOVIES' else 'name'
                        d_k = 'release_date' if category == 'MOVIES' else 'first_air_date'
                        opts = {f"🎬 {r.get(t_k)} ({str(r.get(d_k))[:4]})": r for r in res}
                        sel = st.selectbox("결과", list(opts.keys()))
                        if st.button("✨ 가져오기"):
                            s = opts[sel]; details = get_tmdb_details(s['id'], category); st.session_state.api_data = {'title': s.get(t_k), 'creator': details['creator'], 'date': s.get(d_k), 'img': f"https://image.tmdb.org/t/p/w500{s.get('poster_path')}", 'venue': details['venue'], 'summary': s.get('overview', '')}; st.rerun()
        ...
        with cl:
            ...
            if img_val and img_val != "None": st.image(img_val, use_container_width=True)
            title = st.text_input("제목", value=data.get('title', ''))
            creator = st.text_input("창작자/미디어", value=data.get('creator', ''))
            rel_date = st.text_input("📅 날짜", value=data.get('date', str(date.today())))
            venue = st.text_input("📍 장소/플랫폼", value=data.get('venue', ''))
        with cr:
            summary = st.text_area("📖 소개/내용", value=data.get('summary', ''), height=100)
            brief = st.text_input("📝 요약 (한 줄 평)")
            highlights = st.text_area("✨ 하이라이트", height=100)
            note = st.text_area("🌈 PRISM (태그 포함)", height=100)
            view_date = st.date_input("🍿 기록일", value=date.today())
            if st.button("✅ 기록 저장", use_container_width=True):
                new = {"category": str(category), "title": str(title).strip(), "creator": str(creator).strip(), "rel_date": str(rel_date), "venue": str(venue).strip(), "summary": str(summary).strip(), "brief": str(brief).strip(), "highlights": str(highlights).strip(), "note": str(note).strip(), "img_url": str(img_val).strip(), "img_url2": "", "save_date": str(date.today()), "view_date": str(view_date)}
                get_connection().execute("""INSERT INTO archive (category, title, creator, rel_date, venue, summary, brief, highlights, note, img_url, img_url2, save_date, view_date) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""", tuple(new.values())).connection.commit()
                supabase.table("archive").upsert(new).execute()
                st.cache_data.clear(); st.session_state.api_data = {}; st.success("✅ 저장 완료!"); time.sleep(0.8); st.rerun()
...
with tab_a:
    st.markdown("""<style>
        ...
        .scrap-card { background: #262626; padding: 15px; border-radius: 10px; margin-bottom: 10px; border-left: 5px solid #1E425E; }
        .tag-cloud { display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 20px; padding: 10px; background: #1a1a1a; border-radius: 10px; }
        .tag-item { background: #333; color: #ddd; padding: 2px 10px; border-radius: 15px; font-size: 0.85em; cursor: pointer; }
    </style>""", unsafe_allow_html=True)

    all_df = get_all_data()
    if not all_df.empty:
        all_df['v_dt'] = pd.to_datetime(all_df['view_date'], errors='coerce')
        # [격리] ALL 탭에서는 SCRAP 제외
        ...
        tab_titles = [f"📅 ALL ({len(display_df)})"] + [f"{cat_emojis[c]}{c} ({len(all_df[all_df['category'] == c])})" for c in cat_order]
        ...
        with sub_tabs[0]:
            ...
            sel_y = st.selectbox("📅 연도", years, key="y_sel")
        ...
        for idx, c_name in enumerate(cat_order):
            with sub_tabs[idx + 1]:
                ...
                if c_name == "SCRAP":
                    # --- [SCRAP 전용 주간 대시보드 UI] ---
                    tags = []
                    for n in c_data['note'].dropna(): tags.extend(extract_hashtags(n))
                    if tags:
                        st.markdown('<div class="tag-cloud">' + "".join([f'<span class="tag-item">#{t}</span>' for t in set(tags)]) + '</div>', unsafe_allow_html=True)
                    
                    c_data['week'] = c_data['v_dt'].dt.to_period('W').apply(lambda r: r.start_time)
                    for week in sorted(c_data['week'].unique(), reverse=True):
                        st.markdown(f"#### 🗓️ {week.strftime('%Y-%m-%d')} 주간")
                        w_items = c_data[c_data['week'] == week]
                        for _, row in w_items.iterrows():
                            with st.container():
                                st.markdown(f"""<div class="scrap-card">
                                    <small>{row['view_date']} | {row['venue']}</small><br>
                                    <strong>{row['title']}</strong><br>
                                    <small style="color: #aaa;">{row['brief']}</small>
                                </div>""", unsafe_allow_html=True)
                                if st.button("자세히 보기", key=f"sc_{row['id']}"): show_details(row)
                else:
...
# 팝업 대기 로직 (연동 시 활용)
if "pop_item" in st.session_state:
    item = st.session_state.pop_item
    del st.session_state.pop_item
    show_details(item)
