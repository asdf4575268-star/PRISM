...
with st.sidebar:
    ...
    # secrets.toml에 ADMIN_PASSWORD = "비밀번호" 가 설정되어 있어야 합니다.
...
with tab2:
    if is_admin:
        c_sync1, _ = st.columns([0.2, 0.8])
        with c_sync1:
            if st.button("📤 클라우드 전송", use_container_width=True):
                migrate_to_supabase()
